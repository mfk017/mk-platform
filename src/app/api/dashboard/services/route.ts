...
export async function GET() {
  ...
  const services = await db.service.findMany({
    where: { center_id: session.user.centerId },
...
export async function POST(req: NextRequest) {
  ...
  const { name_en, name_ar, price, original_price, discount_percent, session_count, duration_minutes, category, description_en, description_ar } = body;
